...
def sessions_hist():
    ...
    for data_path in path_list:
        ...
        if not df.empty:
            ...
            # f_weekday
            plt.subplot(2, 1, 1)
            pw = (df['f_weekday'].groupby(df['f_weekday']).count()).reindex(weekday_range, fill_value=0).plot(kind="bar", ylabel='Counts', xlabel='weekday', title=f'{data_path.stem}')
            plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9, wspace=0.4, hspace=0.6)
            pw.set_xticklabels(weekday_labels, rotation=0)

            plt.subplot(2, 1, 2)
            ph = (df['f_hour_of_day'].groupby(df['f_hour_of_day']).count()).reindex(labels_hours, fill_value=0).plot(kind="bar", ylabel='Counts', xlabel='hour of day', title=f'{data_path.stem}')
...
def sessions_hist_all():
    ...
    """
    print('plot session Hist')
    path = fr'{dataframe_dir_users}\user-sessions_features_all.pickle'
    ...
    if not df.empty:
        ...
        # f_weekday
        plt.subplot(2, 1, 1)
        pw = (df['f_weekday'].groupby(df['f_weekday']).count()).reindex(weekday_range, fill_value=0).plot(kind="bar", ylabel='Counts', xlabel='weekday', title='sessions counts')
        plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9, wspace=0.4, hspace=0.6)
        pw.set_xticklabels(weekday_labels, rotation=0)

        plt.subplot(2, 1, 2)
        ph = (df['f_hour_of_day'].groupby(df['f_hour_of_day']).count()).reindex(labels_hours, fill_value=0).plot(kind="bar", ylabel='Counts', xlabel='hour of day', title='sessions counts')
        ph.set_xticklabels(labels_hours, rotation=0)
        plt.show()
...
def sessions_count_user():
    ...
    p = df.plot(kind='barh', xlabel='counts')
